...
@section('title', 'Service Applications & Manual Form Uploads')

@section('content')
<div class="mb-4">
    <h4 class="fw-bold text-dark mb-1">Service Applications & Upload Review</h4>
    <p class="text-secondary small">Review online & manual application uploads, inspect scanned forms, verify documents, and update user service status.</p>
</div>
    ...
    <form action="{{ route('admin.subscriptions') }}" method="GET" class="row g-3">
        <div class="col-md-9">
            <div class="input-group">
                ...
                <input type="text" name="search" class="form-control border-start-0 border-light ps-0" placeholder="Search by client name, email, application ref (DE-2026-XXXX), or service..." value="{{ $search }}">
            </div>
        </div>
        <div class="col-md-3 d-flex gap-2">
            <button type="submit" class="btn text-white rounded-pill px-4 flex-grow-1" style="background-color: #004225;">Search</button>
            @if($search)
...
<!-- Users and Applications Listing -->
@if($users->count() > 0)
        ...
        @foreach($users as $user)
            <div class="card border-0 shadow-sm rounded-4 overflow-hidden bg-white">
                <div class="card-header bg-light border-0 py-3 px-4 d-flex flex-column flex-sm-row justify-content-between align-items-sm-center gap-2">
                    ...
                    <div>
                        <span class="badge bg-success bg-opacity-10 text-success rounded-pill px-3 py-1 text-uppercase small" style="font-size: 11px;">
                            Applications: {{ $user->clientRequests->count() }}
                        </span>
                    </div>
                </div>
                ...
                <div class="card-body p-4">
                            ...
                            @foreach($user->clientRequests as $request)
                                <div class="border rounded-3 p-3 bg-white">
                                    <div class="d-flex flex-wrap justify-content-between align-items-center mb-3 pb-2 border-bottom">
                                        <div>
                                            <span class="font-monospace fw-bold text-success fs-6 me-2" style="color: #004225 !important;">{{ $request->reference_number ?? ('DE-' . $request->id) }}</span>
                                            <span class="fw-bold text-dark fs-6">{{ $request->service_name }}</span>
                                            <small class="text-muted ms-2">• {{ $request->created_at->format('M d, Y H:i') }}</small>
                                        </div>
                                        <div class="d-flex gap-2 align-items-center">
                                            ...
                                            <span class="badge bg-{{ $request->payment_status === 'Paid' ? 'success' : 'secondary' }} bg-opacity-10 text-{{ $request->payment_status === 'Paid' ? 'success' : 'dark' }}">
                                                {{ $request->payment_status }} (${{ number_format($request->amount_paid, 2) }})
                                            </span>
                    ...
                    @else
                        <div class="text-center py-4 bg-light rounded-3">
                            ...
                            <p class="text-secondary small mb-0 mt-1">This user hasn't submitted any service applications yet.</p>
                        </div>
                    @endif
                </div>
            </div>
